...
import os
import argparse
from kpart.utils import time_print
from kpart.__version__ import __version__
...
def get_opts():
    ...
    group.add_argument(
        "-r", "--reference", help="reference contig fasta", required=True
    )
    group.add_argument(
        "-b",
        "--bam",
        help="directory of bam files, contain at least two sorted and indexed bam which were "
        "generated by mapping reads from parents to reference contigs",
        required=True,
    )
    group.add_argument(
        "-k", "--kmer", help="size of kmer, default=21", type=int, default=21
    )
    group.add_argument(
        "-w",
        "--workdir",
        help="directory for temp files and result files, " 'default="workdir"',
        default="workdir",
    )
    group.add_argument(
...
def contig_classifier(in_bam_dir, ref_fa, kmer_size, wrk_dir, threads):
    ...
    for bam in os.listdir(in_bam_dir):
        ...
        if os.path.exists(out_dir):
            ...
            cmd = "%s %s/group_reads_with_bam.py %s %s %d" % (
                py_runner,
                script_dir,
                fn,
                out_dir,
    ...
    for sample in os.listdir(reads_dir):
        ...
        if os.path.exists(out_dir):
            ...
            cmd = "%s %s/get_reads_all_kmers.py %s %d %s %d" % (
                py_runner,
                script_dir,
                fq_dir,
                kmer_size,
    ...
    if os.path.exists(stat_file):
        ...
        cmd = "%s %s/stat_kmer_dist.py %s %s %s %d" % (
            py_runner,
            script_dir,
            ref_kmer_dir,
            qry_kmer_dir,
    ...
    if os.path.exists(classify_file):
        ...
        cmd = "%s %s/classify_result.py %s %s" % (
            py_runner,
            script_dir,
            stat_file,
            classify_file,
